"""
Entrenamiento final STGCN sobre Train + Validación concatenados.

Descarga el dataset desde MinIO, construye el grafo, crea los tensores
espaciotemporales, concatena Train y Val en un único split de entrenamiento
y entrena NUM_EPOCHS épocas fijas con los hiperparámetros óptimos del HPO.
Guarda los pesos del modelo y los datos de Test para evaluación externa.
La evaluación sobre Test se realiza en 12_evaluacion_modelos.py.

Carga:  artefactos/stgcn_hpo.pt  (best_params)
        MinIO → grupo5/final/...  (dataset_final.parquet)
        MinIO → grupo5/cleaned/gtfs_clean_scheduled/...

Guarda: artefactos/stgcn_final.pth
  {
    'model_state_dict':      ...,
    'best_params':           dict,
...
Uso
---
    uv run python src/models/propagacion_estacion/10_entrenamiento_final_stgcn.py
"""
import copy
import gc
import os
...
RUTA_MODELO = Path(__file__).parent / "artefactos" / "stgcn_final.pth"
...
def construir_grafo(df):
    ...
    d_inv       = np.zeros_like(grados)
    mask        = grados != 0
    d_inv[mask] = np.power(grados[mask], -0.5)
    A_norm = d_inv[:, None] * A * d_inv[None, :]  # broadcasting, evita np.diag N×N
    return node_to_idx, torch.tensor(A_norm, dtype=torch.float32), A_norm
...
def main():
    ...
    node_to_idx, A_tensor, A_np  = construir_grafo(df_gtfs)
    del df_gtfs
    gc.collect()
    ...
    dow_test_raw          = X_full[t_test_ini:, :, IDX_DOW].copy()           # (T_test, N)
    temp_extreme_test_raw = X_full[t_test_ini:, :, IDX_TEMP_EXTREME].copy()  # (T_test, N) — binaria 0/1
    ...
    train_loader = DataLoader(DatasetSTGCN(X_trainval, Y_trainval, hl), batch_size=bs, shuffle=True)
    ...
    wandb.init(project=WANDB_PROJECT, name='stgcn-final-trainval', config=best_params)
