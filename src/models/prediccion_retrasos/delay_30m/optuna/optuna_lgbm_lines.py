"""
Optuna — Búsqueda bayesiana de hiperparámetros para LightGBM (trenes con >30 min restantes, por línea)

Solo usa filas donde scheduled_time_to_end >= 1800s, donde target_delay_30m
es una predicción real a 30 minutos (no relleno con el valor de última parada).

Carga el dataset anual de una línea concreta desde:
    pd1/grupo5/aggregations/lines/line=XX/dataset_final.parquet

Uso:
    uv run python src/models/prediccion_retrasos/delay_30m/optuna/optuna_lgbm_lines.py
...
SAMPLE_FRAC        = 1
MIN_TIME_REMAINING = 1800  # Solo trenes con >30 min de viaje restante
...
df = df[df["scheduled_time_to_end"] >= MIN_TIME_REMAINING]
...
def objective(trial: optuna.Trial) -> float:
    ...
    # Log cada trial a WandB como un run independiente
    run = wandb.init(
        project=WANDB_PROJECT,
        name=f"optuna-30m-line{LINE}-trial{trial.number}",
