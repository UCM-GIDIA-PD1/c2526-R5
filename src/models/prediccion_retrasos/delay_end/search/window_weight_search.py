"""
Búsqueda nocturna: ventana temporal + esquema de pesos — delay_end
==================================================================

Optimizaciones vs versión anterior:
  - Todos los meses se descargan UNA SOLA VEZ al inicio (caché en memoria)
  - Iteraciones reducidas a 3000 (vs 6299 óptimas) — suficiente para comparar
...
Duración estimada: ~2-3h (28 fits × ~5-6 min/fit)

Uso:
    uv run python src/models/prediccion_retrasos/delay_end/search/window_weight_search.py
...
NUM_BOOST_ROUND = 3000      # reducido para la búsqueda (óptimo es 6299)
SAMPLE_FRAC     = 0.4     # delay_end tiene ~3x más filas → muestra del 50%
...
OUTPUT_CSV    = "window_weight_search_end_results.csv"
...
def fetch_month(year: int, month: int) -> pd.DataFrame | None:
    ...
    try:
        ...
        df = df[df["scheduled_time_to_end"] < 1800]   # filtro delay_end
        if SAMPLE_FRAC < 1.0:
...
def run_search():
    print("=" * 70)
    print("Búsqueda ventana × peso — delay_end")
    ...
    for win_label, months_2025 in WINDOW_CONFIGS:
        ...
        for scheme in WEIGHT_SCHEMES:
            ...
            run = wandb.init(
                ...
                group="window-weight-search-end",
    ...
    print("\n" + "=" * 70)
    print(f"delay_end completado. Resultados: {OUTPUT_CSV}")
    print("\nTop 5:")
