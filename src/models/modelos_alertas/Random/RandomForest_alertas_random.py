...
N_ITER = 20   # número de combinaciones a probar (= número de runs en W&B)
...
def main():
    ...
    run = wandb.init(entity=ENTITY, project=PROJECT,
                     name="rf_random_baseline", config={"model": "baseline_estratificado"})
    wandb.log({
    ...
    # ── 5. Muestra pequeña para búsqueda de hiperparámetros ───────────────────
    # 5% del train es suficiente para comparar combinaciones (~750K filas)
    ...
    print(f"\n── Probando {N_ITER} combinaciones (1 run W&B cada una) ──")
    ...
    for i, params in enumerate(combinaciones):
        ...
        # Evaluamos en validación completa
        ...
        print(f"  PR-AUC val: {pr_auc_val:.4f}  |  F1 val: {f1_val:.4f}")

        # Cada combinación = 1 run en W&B
        run = wandb.init(
            entity  = ENTITY,
            project = PROJECT,
            name    = f"rf_random_{i+1:02d}",
            config  = params_completos,
        )
        wandb.log({
    ...
    print(f"\nMejor PR-AUC en validación: {mejor_pr_auc:.4f}")
    print(f"Mejores parámetros: {mejores_params}")

    # ── 7. Modelo final con mejores hiperparámetros ────────────────────────────
    # Reentrenamos con train + val completos (como pide el profesor)
    ...
    wandb.log({
        ...
        "feature_importance": wandb.plot.bar(
            ...
            label="feature",
            value="importance",
            title="Feature Importance - RF Random Search",
        ),
    })
    run.finish()
